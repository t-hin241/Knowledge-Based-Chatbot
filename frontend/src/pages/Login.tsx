...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAuth(tokens.access_token, meData.user)
      navigate('/')
    } catch (e: unknown) {
      ...
      <div className="w-full max-w-[360px] animate-fade-up">
        ...
        <div className="bg-zinc-900/50 border border-zinc-800/50 p-8 rounded-3xl backdrop-blur-xl shadow-2xl">
          ...
          <p className="mt-8 text-center text-xs text-zinc-500 font-mono">
            Don't have an account? <Link to="/register" className="text-amber-400 hover:text-amber-300 transition-colors font-bold underline underline-offset-4 decoration-amber-500/30">Sign up</Link>
          </p>
        </div>
      </div>
    </div>
